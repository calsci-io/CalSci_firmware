...
#include "esp_log.h"
#include "../hybrid_sim/hybrid_sim_capture.h"
...
static inline esp_err_t _cmd(st7565_t *d, uint8_t v)
{
    ...
    if (ret != ESP_OK) {
        ...
        hybrid_sim_capture_write_instruction(v);
    }
    return ret;
}
...
void st7565_write_instruction(st7565_t *d, uint8_t c) {
    (void)_cmd(d, c);
}

void st7565_write_data(st7565_t *d, uint8_t v) {
    (void)_send_data(d, &v, 1);
    hybrid_sim_capture_write_byte(v);
}
                           ...
                           uint8_t width, uint8_t pages)
{
    ...
    hybrid_sim_capture_draw_region(buf, page, column, width, pages);
}
